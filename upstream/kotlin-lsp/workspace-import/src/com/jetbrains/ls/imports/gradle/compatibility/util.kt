...
internal fun <T : Comparable<T>> parseRange(data: List<String>, transform: (String) -> T): Ranges<T> {
    ...
    return range(*list.map { parseVersion(it[0], transform) to parseVersion(it[1], transform) }
        .toTypedArray())
}
